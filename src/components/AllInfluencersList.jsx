...
const AllInfluencersList = ({ language, setSelectedInfluencerw }) => {
            ...
            <div key={influencer._id}>
              <IPhoneMockup width={300} orientation="portrait" color="black">
                <div
                  ...
                  style={{
                    backgroundImage: `url(${influencer.photos[0]})`,
                ...
                >
                  ...
                  <p className="bg-black bg-opacity-50 w-full py-1">
                    Followers: {influencer.platforms.reduce((total, p) => total + p.followers, 0)}
                  </p>
                  <button 
                    onClick={() => handleDetailClick(influencer)} 
                    className="bg-black mb-3 font-bold text-white px-6 py-2 rounded-lg hover:bg-black"
                  >
                    {language === 'en' ? "View Detail" : "በዝርዝር ይመልከቱ"}
                  </button>
                  <button 
                    onClick={() => handleBookClick(influencer)} 
                    className="bg-black font-bold text-white px-6 py-2 rounded-lg hover:bg-black"
                  >
